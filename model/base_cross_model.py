...
class PerceiveEncoder(nn.Module):
    ...
    def __init__(self, n_input_channels, n_latent, n_latent_channels=512, n_cross_att_heads=1,
        ...
        self.position_enc = PositionalEncoding(n_input_channels, max_len=900)
        self.dropout = nn.Dropout(p=dropout)
    ...
    def _init_parameters(self):
        with torch.no_grad():
            self.latent.normal_(0.0, 0.02).clamp_(-2.0, 2.0)  # 正态分布

    def forward(self, feats_embedding):
        """
        :param feats_embedding: (bs, n_length, dim) mask (ns,1,nlength)
        :param return_atts:
        :return:
        """
        # print("feats_embedding",feats_embedding.shape)
...
class PerceiveDecoder(nn.Module):
    ...
    def forward(self, query, latent, return_atts=False):
        ...
        # print(query.shape, x_latent.shape)
